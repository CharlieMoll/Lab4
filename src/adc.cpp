// Description: This file implements initialization function for ADC.
//----------------------------------------------------------------------//

#include "pwm.h"

void initADC0() {
    // set reference voltage set to AVCC = 5V output. (we need to measure 5V output from 
    // the development barod to determine what the reference value is for precise calculations
    // since 5V output isn't actually 5V)
    ADMUX |= (1 << REFS0);
    ADMUX &= ~(1 << REFS1);
